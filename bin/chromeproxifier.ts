...
class FetchService {
    ...
    static filterForbiddenHeaders<T extends Record<string, string>>(
        headers: T,
    ) {
    ...
    private async getTarget(url: URL): Promise<TargetInfos> {
        ...
        if (!targetId) {
            ({ targetId } = await this.client.Target.createTarget({
                url: url.href,
            }));
            // Wait a few seconds for the target to be created on the browser's
            // side.
    ...
    async fetch(config: FetchConfig): Promise<FetchResponse> {
        ...
        if (result.subtype === 'error') {
            throw new Error(
                `An error has occured during payload execution: ${
                    result.description
                }`,
            );
        }
