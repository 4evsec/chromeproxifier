...
class FetchService {
  client;
  static payloadTemplate;
  static headersIgnoreReg;
  static {
  ...
  static filterHeaders = pickBy((_, key) => !this.headersIgnoreReg.test(key));
  static getFetchPayload({ url, ...options }) {
    if (options.headers) {
